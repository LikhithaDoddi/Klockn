...
function JoinGroupContent() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [previewRes] = await Promise.all([
          api.get<{ success: boolean; data: GroupPreview }>(`/api/v1/groups/${groupId}/preview`),
        ])
        ...
        if (searchParams.get('joined') === 'true') {
          ...
          try {
            const avRes = await api.get<{ success: boolean; data: GroupAvailability }>(
              `/api/v1/groups/${groupId}/availability`,
              { params: { weekStart } }
            )
    ...
    loadData()
  }, [groupId, searchParams, weekStart])
  ...
  async function handleConnect() {
    ...
    try {
      const res = await api.get<{ success: boolean; data: { url: string } }>(
        `/api/v1/groups/${groupId}/join-link`,
        { params: { platform: 'web' } }
      )
  ...
  if (joined) {
    return (
      <PostJoinScreen
        groupName={groupAvailability?.groupName ?? preview?.name ?? ''}
      ...
      <div className="w-full max-w-sm fade-in-up relative z-10">
        ...
        <div className="glass-card rounded-3xl p-8 shadow-[0_8px_32px_rgba(124,58,237,0.1)] flex flex-col items-center gap-5 text-center">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#7C3AED] to-[#A78BFA] flex items-center justify-center text-white text-3xl font-black shadow-[0_8px_24px_rgba(124,58,237,0.35)]">
            {preview?.name?.[0]?.toUpperCase() ?? 'K'}
          </div>

          <div className="flex flex-col gap-1.5">
            <p className="text-sm text-[#71717A] font-medium">You've been invited to join</p>
            <h1 className="text-2xl font-bold text-[#09090B] tracking-tight">{preview?.name}</h1>
            <div className="flex items-center justify-center gap-3 text-xs text-[#71717A]">
              {preview?.createdBy && <span>Created by {preview.createdBy}</span>}
              {preview?.memberCount != null && (
                <>
                  <span className="w-1 h-1 rounded-full bg-[#D4D4D8] inline-block" />
